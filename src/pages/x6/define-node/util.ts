...
function processingHandle(
  ...
  const { newShape, newNodeId } = other;
  const topData = node.getData();
  const successNodes = graph?.getSuccessors(node);

  let nextNodes = successNodes;
  // 如果当前节点是分支节点，他的邻居节点包含子节点，需要排除掉
  if (node.prop('shape') === 'branch-react-node') {
    nextNodes = successNodes.filter((currentNode) => {
      const nextId = currentNode.prop('id');
...
export function addCondition(graph: Graph, node: Node) {
  graph.batchUpdate(() => {
    const data: { branches: BranchesType[] } & Record<string, any> = node.getData();
    // 获取当前点击节点的 位置
    const position = node.prop('position');
    // 获取当前点击节点的 id
    const topId = node.prop('id');
